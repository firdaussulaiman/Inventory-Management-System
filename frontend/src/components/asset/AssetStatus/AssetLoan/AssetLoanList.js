...
const selectloanAssets = (state) =>
  state.asset.assets.filter((asset) => asset.Status && asset.Status.toLowerCase() === 'loan');

const AssetAssignedList = () => {
  const loanAssets = useSelector(selectloanAssets);
  ...
  const rows = loanAssets.map((asset, index) => ({
    id: asset._id,
    index: index + 1,
    ...asset,
...
export default AssetAssignedList;
